...
import torch
import pickle
from tools.operations import GaussianRenderer
...
def main():
    ...
    for map_id in id_list:
        print(f"generating mesh for gaussian map {map_id}")
        map_id = int(map_id)
        ...
        mesh = generate_mesh(gaussian_map, camera_params)
...
def generate_mesh(gaussian_map, camera_params):
    ...
    for camera_view in camera_params:
        ...
        rgb = torch.clamp(rgb, 0.0, 1.0).squeeze(0).permute(1, 2, 0).cpu().numpy()
        ...
        rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
            rgb, depth, depth_scale=1, depth_trunc=5.0, convert_rgb_to_intensity=False
        )
...
def filter_isolated_vertices(mesh, filter_cluster_min_tri=200):
